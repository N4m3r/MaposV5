...
class OsController extends REST_Controller
{
    ...
    // ==================== MÉTODOS DE ATRIBUIÇÃO DE TÉCNICO ====================

    /**
     * Atribuir técnico a uma OS
     * POST api/v1/os/tecnico/atribuir
     */
    public function atribuirTecnico_post()
    {
        $this->logged_user();
        if (! $this->permission->checkPermission($this->logged_user()->level, 'eOs')) {
            $this->response([
                'status' => false,
                'message' => 'Você não está autorizado a atribuir técnicos',
            ], REST_Controller::HTTP_UNAUTHORIZED);
        }

        $os_id = $this->post('os_id', true);
        $tecnico_id = $this->post('tecnico_id', true);
        $observacao = $this->post('observacao', true);

        if (! $os_id || ! $tecnico_id) {
            $this->response([
                'status' => false,
                'message' => 'OS ID e Técnico ID são obrigatórios',
            ], REST_Controller::HTTP_BAD_REQUEST);
        }
        ...
        $atribuido_por = $this->logged_user()->idUsuarios;

        if ($this->tecnico_model->atribuirTecnico($os_id, $tecnico_id, $atribuido_por, $observacao)) {
            $this->log_app('Atribuiu técnico ' . $tecnico_id . ' à OS #' . $os_id);
            ...
            $this->response([
                ...
                'result' => [
                    ...
                    'tecnico_nome' => $os->nome_tecnico ?? 'N/A',
                ],
            ], REST_Controller::HTTP_OK);
        }
        ...
        $this->response([
            'status' => false,
            'message' => 'Erro ao atribuir técnico. Verifique se já não está atribuído.',
        ], REST_Controller::HTTP_INTERNAL_SERVER_ERROR);
    }

    /**
     * Remover técnico de uma OS
     * POST api/v1/os/tecnico/remover
     */
    public function removerTecnico_post()
    {
        $this->logged_user();
        if (! $this->permission->checkPermission($this->logged_user()->level, 'eOs')) {
            $this->response([
                'status' => false,
                'message' => 'Você não está autorizado a remover técnicos',
            ], REST_Controller::HTTP_UNAUTHORIZED);
        }

        $os_id = $this->post('os_id', true);
        $motivo = $this->post('motivo', true);

        if (! $os_id) {
            $this->response([
                'status' => false,
                'message' => 'OS ID é obrigatório',
            ], REST_Controller::HTTP_BAD_REQUEST);
        }
        ...
        if ($this->tecnico_model->removerTecnico($os_id, $motivo)) {
            $this->log_app('Removeu técnico da OS #' . $os_id);
     ...
     * GET api/v1/tecnicos
     */
    public function listarTecnicos_get()
    {
        $this->logged_user();

        $this->load->model('tecnico_model');
        $tecnicos = $this->tecnico_model->getTecnicosDisponiveis();
        ...
        $this->response([
            'status' => true,
            'message' => 'Lista de técnicos',
            'result' => $result,
        ], REST_Controller::HTTP_OK);
    }

    /**
     * Histórico de atribuições de uma OS
     * GET api/v1/os/tecnico/historico/{os_id}
     */
    public function historicoTecnico_get($os_id = null)
    {
        $this->logged_user();
        if (! $this->permission->checkPermission($this->logged_user()->level, 'vOs')) {
            $this->response([
                'status' => false,
                'message' => 'Você não está autorizado a visualizar histórico',
            ], REST_Controller::HTTP_UNAUTHORIZED);
        }
        ...
        $this->response([
            'status' => true,
            'message' => 'Histórico de atribuições',
            'result' => $historico,
        ], REST_Controller::HTTP_OK);
    }

    /**
     * Obter/Atualizar técnico de uma OS específica
     * GET/PUT api/v1/os/{id}/tecnico
     */
    public function tecnico_get($os_id = null)
    {
        $this->logged_user();
        ...
        $this->response([
            'status' => true,
            'message' => 'Dados do técnico',
            'result' => [
                'os_id' => $os_id,
                'tecnico_id' => $os->tecnico_responsavel,
                'tecnico_nome' => $os->nome_tecnico ?? null,
            ],
        ], REST_Controller::HTTP_OK);
    }
}
