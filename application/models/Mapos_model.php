...
class Mapos_model extends CI_Model
{
    ...
    public function pesquisar($termo, $tipo = 'todos')
    {
        $data = [];

        // Busca apenas em clientes ou em todos
        ...
        // Busca OS - inclui join com clientes para mostrar nome do cliente
        if ($tipo === 'todos' || $tipo === 'os') {
            ...
            $this->db->or_like('os.descricaoProduto', $termo);
            $this->db->or_like('os.defeito', $termo);
            $this->db->or_like('os.status', $termo);
            $this->db->or_like('os.observacoes', $termo);
     ...
     */
    public function getStatusCor($status)
    {
        $cores = [
            'Aberto' => '#f39c12',
            'Em Andamento' => '#00c0ef',
            ...
            'Finalizado' => '#5f9ea0',
            'Cancelado' => '#dd4b39',
            'Aguardando Peças' => '#ff851b',
        ];

        return isset($cores[$status]) ? $cores[$status] : '#777';
    }
}
