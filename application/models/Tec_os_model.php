...
class Tec_os_model extends CI_Model
{
    ...
    public function getServicosOs($os_id)
    {
        ...
        $result = $query->result();
        log_message('info', 'Tec_os_model::getServicosOs - OS ' . $os_id . ' - ' . count($result) . ' serviços encontrados em servicos_os');
        ...
        if (empty($result) && $this->db->table_exists('os_servicos')) {
            ...
            if ($query2 !== false) {
                ...
                log_message('info', 'Tec_os_model::getServicosOs - OS ' . $os_id . ' - ' . count($result) . ' serviços encontrados em os_servicos');
            }
        }
        ...
        if (empty($result)) {
            ...
            if ($simple_query && $simple_query->num_rows() > 0) {
                log_message('error', 'Tec_os_model::getServicosOs - OS ' . $os_id . ' tem ' . $simple_query->num_rows() . ' serviços, mas JOIN falhou. Verifique se os IDs de servico existem na tabela servicos.');
                // Retorna os dados sem o nome do serviço
                $result = $simple_query->result();
                foreach ($result as $r) {
                    $r->servico_nome = 'Serviço #' . $r->servicos_id;
                }
            }
        }
