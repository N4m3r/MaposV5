    ...
    <div class="col-12" style="margin-left: 0">
        <?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'aProduto')) : ?>
            <div class="col-3 flexxn" style="display: flex;">
                <a href="<?= base_url() ?>index.php/produtos/adicionar" class="button btn btn-sm btn-success" style="max-width: 160px">
                    <span class="button__icon"><i class='bx bx-plus-circle'></i></span><span class="button__text2"> Produtos</span>
                </a>
                <a href="#modal-etiquetas" role="button" data-bs-toggle="modal" class="button btn btn-sm btn-warning" style="max-width: 160px">
                    <span class="button__icon"><i class='bx bx-barcode-reader' ></i></span><span class="button__text2">Gerar Etiquetas</span>
                </a>
            </div>
        <?php endif; ?>
        <form class="col-9" method="get" action="<?= base_url() ?>index.php/produtos" style="display: flex; justify-content: flex-end;">
            <div class="col-3">
                <input type="text" name="pesquisa" id="pesquisa" placeholder="Buscar por Nome ou Cod. barra..." class="col-12" value="<?= e($this->input->get('pesquisa')) ?>">
            </div>
            <div class="col-1">
            ...
            <table id="tabela" class="table table-bordered ">
                ...
                foreach ($results as $r) {
                    ...
                    if ($this->permission->checkPermission($this->session->userdata('permissao'), 'dProduto')) {
                        echo '<a style="margin-right: 1%" href="#modal-excluir" role="button" data-bs-toggle="modal" produto="' . $r->idProdutos . '" class="btn-nwe4" title="Excluir Produto"><i class="bx bx-trash-alt bx-xs"></i></a>';
                    }
                    if ($this->permission->checkPermission($this->session->userdata('permissao'), 'eProduto')) {
                        echo '<a href="#atualizar-estoque" role="button" data-bs-toggle="modal" produto="' . $r->idProdutos . '" estoque="' . $r->estoque . '" class="btn-nwe5" title="Atualizar Estoque"><i class="bx bx-plus-circle bx-xs"></i></a>';
                    }
                    if ($this->permission->checkPermission($this->session->userdata('permissao'), 'eProduto')) {
...
<!-- Modal Estoque -->
<div id="atualizar-estoque" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <form action="<?php echo base_url() ?>index.php/produtos/atualizar_estoque" method="post" id="formEstoque">
        ...
        <div class="modal-body">
            ...
            <div class="mb-3">
                <label for="estoque" class="form-label">Adicionar Produtos<span class="required">*</span></label>
                <div class="controls">
                    <input type="hidden" id="idProduto" class="idProduto" name="id" value=""/>
...
<!-- Modal Etiquetas -->
<div id="modal-etiquetas" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <form action="<?php echo base_url() ?>index.php/relatorios/produtosEtiquetas" method="get">
        ...
        <div class="modal-body">
            <div class="col-12 alert alert-info" style="margin-left: 0"> Escolha o intervalo de produtos para gerar as etiquetas.</div>

            <div class="col-12" style="margin-left: 0;">
                <div class="col-6" style="margin-left: 0;">
                    <label for="valor">De</label>
                    <input class="col-9" style="margin-left: 0" type="text" id="de_id" name="de_id" placeholder="ID do primeiro produto" value=""/>
                </div>


                <div class="col-6">
                    <label for="valor">Até</label>
                    <input class="col-9" type="text" id="ate_id" name="ate_id" placeholder="ID do último produto" value=""/>
                </div>

                <div class="col-4">
                    <label for="valor">Qtd. do Estoque</label>
                    <input class="col-12" type="checkbox" name="qtdEtiqueta" value="true"/>
                </div>

                <div class="col-6">
                    <label class="col-12" for="valor">Formato Etiqueta</label>
                    <select class="col-5" name="etiquetaCode">
