...
<body>
    ...
    <div class="card">
        ...
        foreach ($tabelas as $t) {
            $existe = $CI->db->table_exists($t);
            echo $existe
                ? "<div class='tabela-ok'>Tabela <code>$t</code> existe</div>"
                : "<div class='tabela-falta'>Tabela <code>$t</code> <b>FALTA</b></div>";
    ...
    <div class="card">
        ...
        if (in_array('agente_ia_configuracoes', $faltantes)) {
            ...
            foreach ($padroes as $p) {
                runSql($CI, "INSERT INTO `agente_ia_configuracoes` (`chave`, `valor`, `descricao`, `grupo`, `sensivel`)
                    ...
                    ON DUPLICATE KEY UPDATE `valor`=`valor`", 'Seed config: ' . $p[0]);
            }
        }
        ...
        if ($CI->db->trans_status() === FALSE) {
            ...
            echo "<p><a href='" . site_url('agente_ia') . "' class='btn'>Ir para Painel Agente IA</a></p>";
            echo "<p style='color:#e74c3c'><b>IMPORTANTE: Delete o arquivo <code>instalar_agente_ia.php</code> do servidor agora!</b></p>";
        }
        ?>
    </div>
    <?php else: ?>
